...
func (s *sqlRepo) GetRestaurantLike(ctx context.Context, ids []int) (map[int]int, error) {
	...
	if err := s.db.Table(entity.Like{}.TableName()).Select("restaurant_id, count(restaurant_id) as count").Where("restaurant_id in (?)", ids).Group("restaurant_id").Find(&listLike).Error; err != nil {
...
func (s *sqlRepo) GetUsersLikeRestaurant(ctx context.Context, conditions map[string]interface{}, filter *entity.Filter, paging *common.Paging, moreKeys ...string) ([]common.SimpleUser, error) {
	var result []entity.Like

	db := s.db

	db = db.Table(entity.Like{}.TableName()).Where(conditions)
